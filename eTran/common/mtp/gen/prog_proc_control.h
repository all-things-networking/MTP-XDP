...
static inline struct mtp_ev_list parse_tcp(pkt_t p, struct ipv4 h)
{
    ...
    if ((bp.flags & MTP_FLAG_SYN) != 0 && (bp.flags & MTP_FLAG_ACK) == 0) {
        struct tcp_syn s;
        s.seq = bp.seq_no;
        s.sport = bp.src_port;
        s.dport = bp.dst_port;
        ...
        s.ece = (bp.flags & MTP_FLAG_ECE) != 0;
        s.cwr = (bp.flags & MTP_FLAG_CWR) != 0;
        s.qid = p.qid;
        mtp_ev_key(&s, MTP_CTX_tcp_listen_ctx, &lid);
        mtp_ev_add(&out, &s);
        return out;
    }
    if ((bp.flags & MTP_FLAG_SYN) != 0) {
        ...
        sa.ece = (bp.flags & MTP_FLAG_ECE) != 0;
        sa.cwr = (bp.flags & MTP_FLAG_CWR) != 0;
        sa.qid = p.qid;
        mtp_ev_key(&sa, MTP_CTX_tcp_ctx, &fid);
        mtp_ev_add(&out, &sa);
        return out;
    }
    if ((bp.flags & MTP_FLAG_RST) != 0) {
        struct tcp_rst r;
        r.seq = bp.seq_no;
        r.ack = bp.ack_seq;
    ...
    a.ecn_ce = (h.tos & MTP_IP_ECN_CE) == MTP_IP_ECN_CE;
    ...
    if (payload_len > 0) {
        ...
        d.ecn_ce = (h.tos & MTP_IP_ECN_CE) == MTP_IP_ECN_CE;
...
static inline void proc_bind(struct app_bind *ev, struct tcp_ctx_common *ctx_common, struct tcp_ctx_control *ctx_control)
{
    ...
    ctx_control->type = MTP_TYPE_FAKE;
...
static inline void proc_listen(struct app_listen *ev, struct tcp_ctx_common *bound_common, struct tcp_ctx_control *bound_control, struct tcp_listen_ctx_common *lst_common)
{
    if (bound_control->type != MTP_TYPE_FAKE) {
        return;
    }
...
static inline void proc_synack(struct tcp_synack *ev, struct tcp_ctx_common *ctx_common, struct tcp_ctx_control *ctx_control, struct tcp_ctx_ebpf *ctx_ebpf)
{
    ...
    bp.ack_seq = ctx_ebpf->rx_next_seq;
    bp.window = MTP_PARITY_CTRL_WINDOW;
    bp.flags = MTP_FLAG_ACK;
    mtp_opt_add(bp.opts.opts, &bp.opts.opts_n, opt_ts(0, ctx_control->syn_ts));
    mtp_pkt_gen(&bp, MTP_PRIO_ACK, false);
}
