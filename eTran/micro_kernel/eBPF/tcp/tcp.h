...
static __always_inline int enqueue_prev_ack(__u32 cpu)
{
    ...
    ack->seq = c->mtp.tx_next_seq;
    ack->ack = c->mtp.rx_next_seq;

    ack->rxwnd = min(c->mtp.rx_avail >> TCP_WND_SCALE, 0xFFFF);
    
    ack->ts_val = now;
    ack->ts_ecr = c->mtp.tx_next_ts;
    c->mtp.tx_next_ts = 0;
...
{
    return c->mtp.rx_remote_avail;
}

static __always_inline __u32 tcp_txavail(const struct bpf_tcp_conn *c)
{
    /* flow control window */
    return c->mtp.rx_remote_avail - c->mtp.tx_sent;
}
...
{
    __u32 tx_seq = c->mtp.tx_next_seq;
    __u32 rx_wnd = c->mtp.rx_avail;
    __u32 ack_seq = c->mtp.rx_next_seq;
    __u32 ts_ecr = c->mtp.tx_next_ts;
...
static __always_inline __u64 cc_get_desired_tx_ts(struct bpf_cc *cc, __u64 ref_ts, __u32 payload_len)
{
    ...
    __u64 ns_delta = (__u64)1000000000 * payload_len / cc->mtp.rate;
...
{
    __u32 go_back_bytes = c->mtp.tx_sent;
    __u32 x;

    /* reset flow state as if we never transmitted those segments */
    c->mtp.rx_dupack_cnt = 0;

    c->mtp.tx_next_seq -= go_back_bytes;
    if (c->mtp.tx_next_pos >= go_back_bytes) {
        c->mtp.tx_next_pos -= go_back_bytes;
    } else {
        x = go_back_bytes - c->mtp.tx_next_pos;
        c->mtp.tx_next_pos = c->mtp.tx_buf_size - x;
    }

    c->mtp.tx_pending = 0;
    c->mtp.rx_remote_avail += go_back_bytes;

    c->mtp.tx_sent = 0;
    cc->mtp.txp = 0;

    /* cut rate by half if first drop in control interval */
    if (cc->mtp.cnt_tx_drops == 0) {
        cc->mtp.rate >>= 1;
    }

    cc->mtp.cnt_tx_drops++;

    return c->mtp.tx_next_pos;
}

// Caller must hold bpf_spin_lock
/*
 * tcp_tx_process() is GONE -- ported as dispatch_tcp_tx in
 * mtp/tcp_program_bpf.h. Its locals are now an MTP scratchpad, and the three
 * events that share this one execution site are named there.
 */

static __always_inline int tcp_valid_rxack(struct bpf_tcp_conn *c, __u32 ack_seq, __u32 *bump)
{
    __u32 exp_ack_first = c->mtp.tx_next_seq - c->mtp.tx_sent;
    __u32 exp_ack_last = c->mtp.tx_next_seq;

    // allow receving ack that we haven't sent yet, this is probably caused by retransmission
    exp_ack_last += c->mtp.tx_pending;
...
{
    __u32 exp_seq_first = c->mtp.rx_next_seq;
    __u32 exp_seq_last = c->mtp.rx_next_seq + c->mtp.rx_avail;
...
/*
 * tcp_rx_process() is GONE -- ported as dispatch_tcp_rx in
 * mtp/tcp_program_bpf.h, with its fifteen locals named as an MTP scratchpad and
 * each of the event documents' processors marked in place.
 */
